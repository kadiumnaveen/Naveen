...
public class IDINotificationManager {
	...
	protected static void updatecatche() {
		try {
			stmt=conn.prepareStatement("select IDI_IFSC_CODE,ROWID from IDI_IFSC_DIR_INFO where ROWID='"+rowid+"'");
			rs=stmt.executeQuery();
	        while(rs.next()) {
	...
	public static void loadcatche() {
		...
		try {
	        ...
	        while(rs.next()) {
	        	...
	        	System.out.println("Update data related to IDI_IFSC_CODE : "+rs.getString(1));
	        }
	        printCatche();
		}catch(Exception e) {
			e.printStackTrace();
	...
	public static OracleConnection connect() throws SQLException
	  {
	    ...
	    Properties prop = new Properties();
	    prop.setProperty("user",IDINotificationManager.USERNAME);
	    prop.setProperty("password",IDINotificationManager.PASSWORD);
	    return (OracleConnection)dr.connect(IDINotificationManager.URL,prop);
	  }
