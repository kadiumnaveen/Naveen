...
public class DBListener implements DatabaseChangeListener
{
  ...
  DBListener(IDINotificationManager2 idiNotificationManager2)
  {
  ...
  public static void deletecatche() {
	...
	protected static void updatecatche() {
		try {
			...
			stmt=IDINotificationManager2.conn.prepareStatement("select IDI_TRNG_FLG,ROWID from IDI_IFSC_DIR_INFO where ROWID='"+rowid+"'");
			rs=stmt.executeQuery();
			ResultSetMetaData rsmd=rs.getMetaData();
			System.out.println(rsmd.getColumnTypeName(1)+"COLUMN TYPE ");
			if(rs!=null) {
	        while(rs.next()) {
	        	System.out.println("sdfgsdfgsdfgjpiosdjfngsoijd");
	        	//System.out.println(rs.getRef(1).toString()); 
