...
public class Worker2NotificationManager {
	
	  static String USERNAME= "system";
	  static String PASSWORD= "system123";
	  static String URL="jdbc:oracle:thin:@localhost:1521:KNAV";
	...
	protected static void updatecatche() {
		try {
			stmt=conn.prepareStatement("select WORKER_ID,ROWID from Workers where ROWID='"+rowid+"'");
			rs=stmt.executeQuery();
	        while(rs.next()) {
	...
	public static OracleConnection connect() throws SQLException
	  {
	    ...
	    Properties prop = new Properties();
	    prop.setProperty("user",Worker2NotificationManager.USERNAME);
	    prop.setProperty("password",Worker2NotificationManager.PASSWORD);
	    return (OracleConnection)dr.connect(Worker2NotificationManager.URL,prop);
	  }
