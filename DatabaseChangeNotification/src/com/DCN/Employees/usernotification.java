...
public class usernotification {
	...
	public static void startListening() throws Exception {
	    ...
	    try {
	    	...
	    	Statement stmt = conn.createStatement();
	         ...
	         ResultSet rs = stmt.executeQuery("select USER_ID from SFMSUSER");
	...
	protected static void updatecatche() {
		try {
			stmt=conn.prepareStatement("select USER_ID,ROWID from SFMSUSER where ROWID='"+rowid+"'");
			rs=stmt.executeQuery();
	        while(rs.next()) {
	        	IDI_IFSC_values.put(rs.getString("ROWID"),rs.getString("USER_ID"));
	        	System.out.println("Update data related to user_ID : "+rs.getString(1));
	        }
	        printCatche();
		}catch(Exception e) {
			e.printStackTrace();
	...
	public static void loadcatche() {
		...
		try {
			...
			rs=stmt.executeQuery();
	        while(rs.next()) {
	        	IDI_IFSC_values.putIfAbsent(rs.getString("ROWID"),rs.getString("USER_ID"));
	        	System.out.println("Update data related to userid : "+rs.getString(1));
	        }
	        printCatche();
		}catch(Exception e) {
			e.printStackTrace();
	...
	public static OracleConnection connect() throws SQLException
	  {
	    ...
	    Properties prop = new Properties();
	    prop.setProperty("user",usernotification.USERNAME);
	    prop.setProperty("password",usernotification.PASSWORD);
	    return (OracleConnection)dr.connect(usernotification.URL,prop);
	  }
